...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      ...
      <section className="official-section" id="official" aria-labelledby="official-title"><div className="official-header"><div><p className="eyebrow">01 / FIXED OBSERVATION</p><h2 id="official-title">公式定点観測</h2></div><p>運営側が固定した条件を、同じ方法で継続観測する。<br />検証runとライブ実験の結果は、公開時系列へ混ぜない。</p></div><div className="official-score"><div className="score-copy">{official.isBaseline && <span className="baseline-badge">初回ベースライン / 正式観測</span>}<span className="score-label">暫定スコア</span>{official.hasObservation && official.score !== null ? <strong>{official.score}<small>/100</small></strong> : <strong className="empty-score">—</strong>}<span className="score-note">{official.hasObservation ? `最終観測 ${formatDate(official.observedAt)}` : '初回観測後に表示'}</span>{official.hasObservation && <span className="score-coverage">{official.metricCoverage.available}/{official.metricCoverage.total}指標で算出 · 持続性は次回以降に評価</span>}</div><div className="score-ring"><div><span>{official.hasObservation ? `${official.score}` : '—'}</span><small>{official.hasObservation ? 'PROVISIONAL' : 'NO DATA'}</small></div></div><div className="official-stats"><div><span>正式観測回数</span><strong>{official.completedRuns || '—'}</strong></div><div><span>正式累計解析ページ</span><strong>{official.analyzedPages ? official.analyzedPages.toLocaleString() : '—'}</strong></div><div><span>基準日</span><strong>{official.startDate ? formatDate(official.startDate).split(' ')[0] : '—'}</strong></div></div></div></section>

      <section className="method-section" id="method" aria-labelledby="method-title"><SectionTitle number="02" eyebrow="HOW WE READ THE RESULT">4つの指標で、情報の構造を読む。</SectionTitle><div className="metric-grid">{['独自性', '出典健全性', '発見多様性', '持続性'].map((label, index) => <article className="metric-card" key={label}><div className="metric-card-head"><span className="metric-no">0{index + 1}</span><h3>{label}</h3><span className="metric-status">推定</span></div><MetricVisual type={label} /><p>{label === '独自性' ? '同じ情報がどれだけ繰り返されているかではなく、内容の新しさを見ます。' : label === '出典健全性' ? '情報の出どころが明確で、検証可能かどうかを見ます。' : label === '発見多様性' ? '異なるドメインや運営主体から情報に出会えるかを見ます。' : '前回の正式観測と正規化URLを比較し、消失・追加・順位変動を記録。ページ取得が有効になるまで、リダイレクト・取得不能・本文変更・本文ハッシュは未計算です。'}</p><span className="metric-bottom">{official.hasObservation ? '観測データあり' : '観測後に表示'} <Icon name="arrow" size={14} /></span></article>)}</div></section>
