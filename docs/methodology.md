...
Live search is a separate product cache and is never mixed into official observation counts. It reports search-result retrieval, bounded page-body retrieval, snippet-only results, primary-source evaluability, and failure reasons separately. `snippet_only` means no page body was available; similarity groups and information lineages are provisional and based on snippets, and primary-source assessment is `Not evaluable`. `mixed_content` means 1〜9 page bodies were retrieved, and `full_content` means every returned result had bounded body evidence. Top 20 pagination is intentionally not requested in live or official search when the provider returns fewer than 20.
