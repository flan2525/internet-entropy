...
Brave Search APIの当該実行のアプリケーション側消費量は、固定クエリ数と `apiRequests` で確認する。`count=20` はリクエストへ含めるが、主観測はTop 10であり、11〜20件の追加paginationは行わない。1検索語診断は `.github/workflows/brave-diagnostic.yml` から `OBSERVATION_CRON_SECRET` で保護されたPages `/api/admin/diagnose` を呼び、Pages側の `BRAVE_SEARCH_API_KEY` を使ってWeb results件数、HTTPステータス、Top 20 availabilityだけを返す。GitHubへBrave APIキーを登録・複製しない。APIキー・Cron Secret・検索結果本文・タイトルはログ、レスポンス、静的配信物へ出さない。
